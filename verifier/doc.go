...
//
// Wildcard-synthesised positive answers. When the covering RRSIG's
// Labels field is fewer than the qname's label count, the answer
// was produced by wildcard expansion (RFC 4034 §3.1.3). The
// validator reconstructs the wildcard owner for digest computation
// and additionally requires a signed NSEC / NSEC3 proof that the
// next-closer name does not exist (RFC 4035 §5.3.4). On success
// the verdict stays [VerdictSecure] and [Result.Wildcard] carries
// the reconstructed wildcard owner, closest encloser, next-closer
