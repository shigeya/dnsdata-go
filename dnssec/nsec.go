...
// EncodeTypeBitmap delegates to [wire.EncodeTypeBitmap]. The function is
// retained here so existing callers (and the public surface used by
// NSEC3 / CSYNC tests) keep working; new code should call the wire
// version directly.
func EncodeTypeBitmap(rrtypes []uint16) []byte {
	return wire.EncodeTypeBitmap(rrtypes)
}

// DecodeTypeBitmap delegates to [wire.DecodeTypeBitmap], translating
// [wire.ErrTypeBitmap] into the dnssec-package [ErrPresentationFormat]
// sentinel so existing error-classification callers continue to work.
func DecodeTypeBitmap(bitmap []byte) ([]uint16, error) {
	out, err := wire.DecodeTypeBitmap(bitmap)
	if err != nil {
		return nil, fmt.Errorf("%w: NSEC bitmap: %v", ErrPresentationFormat, err)
	}
	return out, nil
}
